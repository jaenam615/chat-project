...
/*
    @CacheEvict: Value에 맞는 캐시가 삭제, DB와 캐시간 간극을 줄임
    @Cacheable: 캐시 데이터를 저장 및 조회하는데 사용됨, 정해진 순서대로 데이터를 관리
    @Caching: 위 두 어노테이션을 동적으로 적용할 수 있다 / 여러 개 적용할 수 있다
*/
...
class ChatServiceImpl(
    ...
    @CacheEvict(cacheNames = ["chatRooms"], allEntries = true)
    override fun createChatRoom(
        ...
        val creator =
            (userRepository.findById(createdBy))
                .orElseThrow { IllegalArgumentException("chat room does not exist") }
    ...
    override fun getChatRooms(
        userId: Long,
        pageable: Pageable,
    ): Page<ChatRoomDto> = chatRoomRepository.findUserChatRooms(userId, pageable).map { chatRoomToDto(it) }
    ...
    override fun getChatRoomMembers(roomId: Long): List<ChatRoomMemberDto> =
        chatRoomMemberRepository.findByChatRoomIdAndIsActiveTrue(roomId).map { memberToDto(it) }
    ...
    override fun sendMessage(
        ...
        chatRoomMemberRepository
            .findOneByChatRoomIdAndUserIdAndIsActiveTrue(request.chatRoomId, senderId)
        ...
        try {
            redisMessageBroker.broadcastToRoom(
                ...
                message = chatMessage,
                excludedServerId = redisMessageBroker.getServerId(),
            )
        } catch (e: Exception) {
    ...
    override fun getMessages(
        ...
        return messageRepository.findByChatRoomId(roomId, pageable).map { messageToDto(it) }
    }
        ...
        val messages =
            when {
                ...
                request.direction == MessageDirection.BEFORE -> {
                    ...
                    messageRepository.findMessagesAfter(request.chatRoomId, cursor, pageable).reversed()
                }
            }
    ...
    @Cacheable(cacheNames = ["chatRooms"], key = "#chatRoom.id")
    open fun chatRoomToDto(chatRoom: ChatRoom): ChatRoomDto {
    ...
    open fun userToDto(user: User): UserDto =
        UserDto(
            id = user.id,
